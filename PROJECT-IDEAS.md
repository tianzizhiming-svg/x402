...
* **What it does:** Provides China-facing digital capabilities via x402 paid endpoints on Base chain — raw-content (web fetch to Markdown) and verified-analysis (human-authored employment/teaching reports for Chinese universities). Live on mainnet at api.060504.shop with MCP server integration and Bazaar discovery support.
* **Payment moments:** Per-call for raw-content ($0.003–$0.008 USDC) or per-report for verified-analysis ($90 USDC).
...
* **Payment moments:** Pay-per-call ($0.003 USDC for static pages, $0.008 USDC for dynamic/JS-rendered pages).
